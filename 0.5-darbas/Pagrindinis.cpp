...
void failuKurimas(vector <int>& v1, int& skaiciai)
{
    string failoPav;
    double pazymys;
    std::stringstream ss;
    for (int i = 0; i < (v1.size()); i++)
    {
        ...
        vector<int> skaiciai;
        isvedimas << setw(20) << left << "Vardas"
            << setw(20) << left << "Pavarde"
            << setw(20) << left << "Galutinis(vid.)" << endl;
        for (int k = 1; k < v1.at(i); k++) {
            skaiciai = auto_marks(5);
            isvedimas << setw(20) << "Vardas" + std::to_string(k) <<
                setw(20) << "Pavarde" + std::to_string(k) <<
        ...
        cout << v1.at(i) << " elementu pildymas truko: " << diff.count() << " s" << endl;
    }
}
